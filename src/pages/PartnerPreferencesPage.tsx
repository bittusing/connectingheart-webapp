  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        await fetchLookup()
        setDataLoaded(true)
        const countryOptions = await api.get<{ label: string; value: string }[]>('/api/lookup/getCountryLookup')
      } catch (error) {}
    }
  }, [])
    ...
    const checkScreenName = async () => {
      try {
        ...
        if (userResponse.status === 'success' && userResponse.data?.screenName) {
          ...
          // If screenName is not partnerpreferences, redirect to correct page
          if (screenName !== 'partnerpreferences' && routeMap[screenName]) {
            navigate(routeMap[screenName], { replace: true })
          }
        }
    ...
    <FormPageLayout title="Partner Preferences" subtitle="Share your partner preferences.">
      <form className="space-y-5" onSubmit={handleSubmit}>
        <TextInput
          ...
          value={formData.partnerPreferences}
          onChange={(event) =>
            setFormData((prev) => ({ ...prev, partnerPreferences: event.target.value }))
          }
        />
        <div className="flex flex-col gap-3">
          <Button type="submit" size="lg" fullWidth disabled={submitting}>
